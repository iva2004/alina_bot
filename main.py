...
from aiogram.fsm.storage.base import StorageKey  # <--- ДОБАВЬТЕ ЭТУ СТРОКУ
...
from bot.keyboards import (
    ...
    get_admin_order_statuses_kb, get_user_main_kb  # <--- ДОБАВЬТЕ ЭТО ИМЯ
)
...
class AdminSettings(StatesGroup):
    ...
    waiting_for_site_name_edit = State()
    waiting_for_site_url_edit = State()
...
class UserOrderStates(StatesGroup):
    waiting_for_user_reply = State() # Ожидание текста ответа от клиента
...
async def cmd_start(message: Message, db_user: User):
    if db_user.is_admin:
        await message.answer("Привет, Админ!", reply_markup=get_user_main_kb(is_admin=True))
    else:
        await message.answer("Привет! Я твой помощник.", reply_markup=get_user_main_kb(is_admin=False))
...
async def admin_manage_single_site(callback: CallbackQuery):
    ...
    async with async_session() as session:
        # Важно: используем ту модель, которая у вас в коде (SiteSetting или Site)
    ...
    builder.button(text="🔗 Изменить URL", callback_data=f"edit_url_{site.id}")  # ДОБАВИЛИ ЭТУ КНОПКУ
...
# --- ШАГ 1: Запрос нового URL ---
@dp.callback_query(F.data.startswith("edit_url_"))
async def edit_site_url_start(callback: CallbackQuery, state: FSMContext):
    # Разделяем "edit_url_5". Элементы будут: [0]"edit", [1]"url", [2]"5"
    # Поэтому берем последний элемент [-1]
    try:
        site_id = int(callback.data.split("_")[-1])
    except (ValueError, IndexError):
        return await callback.answer("❌ Ошибка в данных кнопки")

    await state.update_data(edit_site_id=site_id)

    await callback.message.answer(
        "📥 <b>Введите новый URL для сайта:</b>\n"
        "(Например: <code>https://zara.com</code>)",
        parse_mode="HTML"
    )
    await state.set_state(AdminSettings.waiting_for_site_url_edit)
    await callback.answer()


# --- ШАГ 2: Сохранение в базу ---
@dp.message(AdminSettings.waiting_for_site_url_edit)
async def edit_site_url_save(message: Message, state: FSMContext):
    ...
    async with async_session() as session:
        # ВАЖНО: Используем SiteSetting, так как она была в вашем рабочем manage_site_
        await session.execute(
            update(SiteSetting).where(SiteSetting.id == site_id).values(url=new_url)
        )
        await session.commit()

    await state.clear()
    await message.answer(f"✅ <b>Успешно!</b>\nURL изменен на: <code>{new_url}</code>", parse_mode="HTML")

    # Чтобы админу не пришлось заходить заново, отправляем его в список категорий
    # (Замените на вашу функцию вызова меню, если она называется иначе)
    try:
        await admin_mod_sites_list(message)
    except:
        pass
...
async def cancel_handler(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer("🚫 Действие отменено. Возвращаюсь в главное меню.", reply_markup=get_admin_main_kb())
...
@dp.message(F.text.startswith("📑 Заказы"), StateFilter("*"))
...
async def admin_search_process(message: Message, state: FSMContext):
    ...
    async with async_session() as session:
        # Используем sqlalchemy.or_ напрямую для гарантии
        import sqlalchemy
        stmt = select(Order, User).join(User, Order.user_id == User.id).where(
            sqlalchemy.or_(
    ...
    if not data:
        ...
        order, user = data
        await message.answer(f"✅ <b>Заказ найден!</b>", parse_mode="HTML")
        await send_order_card(message, order, user)
...
async def send_order_card(message_or_query, order: Order, user: User):
    """Универсальная функция отрисовки карточки заказа"""
    status_db = order.status.upper()
    ...
    # 2. Операционные кнопки по статусам (Ваша логика)
    if status_db == "НОВЫЙ":
    ...
    # 3. Блок управления
    builder.button(text="❓ Уточнить детали", callback_data=f"ask_client_{order.id}")
    builder.button(text="✏️ Изменить данные", callback_data=f"edit_order_data_{order.id}")
    builder.button(text="📊 Сменить статус", callback_data=f"manage_order_{order.id}")

    builder.adjust(2, 2, 2, 2)

    # Определяем, как отправить: новым сообщением или редактированием
    if isinstance(message_or_query, CallbackQuery):
        await message_or_query.message.answer(text, reply_markup=builder.as_markup(), parse_mode="HTML")
    else:
...
async def admin_view_single_order(callback: CallbackQuery):
    order_id = int(callback.data.split("_")[-1])

    async with async_session() as session:
        stmt = select(Order, User).join(User, Order.user_id == User.id).where(Order.id == order_id)
        result = await session.execute(stmt)
        data = result.first()
    ...
    await send_order_card(callback, data[0], data[1])
...
async def admin_orders_list(callback: CallbackQuery):
    ...
    for order, user in orders_data:
        # Просто вызываем ту же функцию для каждого заказа в списке!
        await send_order_card(callback, order, user)

# 1. Админ нажимает "Уточнить детали"
@dp.callback_query(F.data.startswith("ask_client_"))
async def admin_ask_client_start(callback: CallbackQuery, state: FSMContext):
    order_id = int(callback.data.split("_")[-1])
    await state.update_data(ask_order_id=order_id)

    # ИСПРАВЛЕНО: Добавлена буква f перед строкой
    await callback.message.answer(
        f"📝 <b>Напишите ваш вопрос клиенту по заказу №{order_id}:</b>\n"
        "(Например: 'Какой именно цвет нужен?' или 'Размера нет, выберите другой')",
        parse_mode="HTML"
    )
    await state.set_state(AdminSettings.waiting_for_admin_question)
    await callback.answer()


# 2. Бот пересылает вопрос клиенту
# 2. Бот пересылает вопрос клиенту
@dp.message(AdminSettings.waiting_for_admin_question)
async def admin_send_question_to_user(message: Message, state: FSMContext):
    data = await state.get_data()
    order_id = data.get("ask_order_id")

    async with async_session() as session:
        # Ищем заказ и пользователя
        order = await session.get(Order, order_id)
        if not order:
            return await message.answer("❌ Заказ не найден в базе.")
    ...
    try:
        msg_text = (f"✉️ <b>Вопрос от менеджера по заказу №{order_id}:</b>\n\n"
                    f"<i>{message.text}</i>")

        kb = InlineKeyboardBuilder()
        kb.button(text="✍️ Ответить менеджеру", callback_data=f"user_reply_{order_id}")

        # ИСПРАВЛЕНО: Используем message.bot вместо просто bot
        ...
        await message.answer(f"✅ Сообщение по заказу №{order_id} отправлено клиенту!")
    except Exception as e:
...
# --- ШАГ 1: Пользователь нажимает "Ответить менеджеру" ---
@dp.callback_query(F.data.startswith("user_reply_"))
async def user_reply_start(callback: CallbackQuery, state: FSMContext):
    order_id = callback.data.split("_")[-1]
    await state.update_data(reply_to_order_id=order_id)

    await callback.message.answer(f"✍️ <b>Введите ваш ответ по заказу №{order_id}:</b>", parse_mode="HTML")
    await state.set_state(UserOrderStates.waiting_for_user_reply)
    await callback.answer()


# --- ШАГ 2: Бот пересылает ответ пользователя админу ---
@dp.message(UserOrderStates.waiting_for_user_reply)
async def user_send_reply_to_admin(message: Message, state: FSMContext):
    data = await state.get_data()
    order_id = data.get("reply_to_order_id")
    user_name = message.from_user.full_name

    # Текст для админа
    admin_text = (
        f"📩 <b>Получен ответ от клиента по заказу №{order_id}</b>\n"
        f"👤 Клиент: {user_name}\n"
        f"───────────────────\n"
        f"💬 Текст: <i>{message.text}</i>"
    )

    # Кнопка для админа, чтобы сразу открыть этот заказ
    kb = InlineKeyboardBuilder()
    kb.button(text="📂 Посмотреть заказ", callback_data=f"manage_order_{order_id}")
    ...
    await state.clear()
    await message.answer("✅ Ваш ответ отправлен менеджеру. Ожидайте уведомления.")
...
async def confirm_weight_payment(callback: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    # Извлекаем ID (проверьте индекс, обычно это последний элемент)
    ...
    # 2. Переключаем состояние КЛИЕНТА (StorageKey должен быть импортирован вверху файла!)
    ...
    # 3. ОТПРАВЛЯЕМ ЗАПРОС КЛИЕНТУ И ОБНОВЛЯЕМ АДМИНКУ
    shipping_msg = (
    ...
    # --- ВОТ ЭТОТ БЛОК, КОТОРЫЙ ВЫ ПРИСЛАЛИ ---
    try:
        # Пишем клиенту
        await bot.send_message(user_data.tg_id, shipping_msg, parse_mode="HTML")

        # Обновляем сообщение у админа (красиво меняем текст на подтверждение)
        if callback.message.caption or callback.message.photo:
            # Если это было фото чека - редактируем подпись под фото
            await callback.message.edit_caption(
                caption=f"✅ Оплата ВЕСА №{oid} принята. Клиенту {user_data.full_name} отправлен запрос реквизитов НП."
            )
        else:
            # Если это было просто текстовое сообщение - редактируем текст
            await callback.message.edit_text(
                text=f"✅ Оплата ВЕСА №{oid} принята. Клиенту {user_data.full_name} отправлен запрос реквизитов НП."
            )
    except Exception as e:
...
async def admin_invoice_start(callback: CallbackQuery, state: FSMContext):
    # Используем [-1] для надежности извлечения ID
    order_id = int(callback.data.split("_")[-1])

    async with async_session() as session:
        # Получаем заказ, чтобы узнать текущую сумму
        order = await session.get(Order, order_id)
    ...
    # Подтягиваем сумму из заказа (из поля price_uah)
    current_price = order.price_uah or 0.0

    await callback.message.answer(
        f"💰 <b>Заказ №{order_id}</b>\n"
        f"Текущая стоимость в заказе: <code>{current_price}</code> грн.\n\n"
        f"Введите итоговую сумму к оплате (грн).\n"
        f"<i>(Если сумма верна, просто скопируйте число выше и отправьте)</i>",
        parse_mode="HTML"
    )
...
async def process_invoice_send(message: Message, state: FSMContext, bot: Bot):
    # 1. ТЕХКОНТРОЛЬ ВВОДА: чистим текст от мусора
    # Убираем пробелы, грн, меняем запятую на точку
    raw_text = message.text.replace(",", ".").replace("грн", "").replace(" ", "").strip()

    try:
        amount = round(float(raw_text), 2)  # Округляем до копеек для порядка
    except ValueError:
        return await message.answer("⚠️ <b>Ошибка!</b> Введите сумму числом (например: <code>4031.42</code>)",
                                    parse_mode="HTML")

    data = await state.get_data()
    # Пытаемся достать ID заказа из любого возможного ключа
    order_id_raw = data.get('inv_order_id') or data.get('current_order_id')

    if not order_id_raw:
        await state.clear()
        return await message.answer("❌ <b>Ошибка:</b> Данные о заказе потеряны. Попробуйте нажать кнопку заново.")

    oid = int(order_id_raw)

    # 2. РАБОТА С БАЗОЙ
    async with async_session() as session:
        # Загружаем заказ вместе с пользователем одним запросом
        stmt = select(Order, User).join(User, Order.user_id == User.id).where(Order.id == oid)
        res = await session.execute(stmt)
        result = res.first()
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="📋 Показать реквизиты", callback_data=f"show_requisites_{oid}")  # Новая кнопка
    ...
    try:
        ...
        # Отвечаем админу
        await message.answer(
            f"✅ Счет на {amount} грн отправлен клиенту.\nЗаказ №{oid} переведен в статус <b>'ОЖИДАЕТ ОПЛАТЫ'</b>.",
            parse_mode="HTML")
    except Exception as e:
        await message.answer(f"❌ Не удалось отправить пуш-уведомление клиенту (возможно, бот заблокирован): {e}")

    await state.clear()

@dp.callback_query(F.data.startswith("show_requisites_"))
async def show_pay_details(callback: CallbackQuery):
    pay_details = (
        "✅ <b>Реквизиты для оплаты заказов:</b>\n\n"
        "💳 Mono: <code>4444 5555 6666 7777</code>\n"
        "👤 Получатель: Иван Иванович И.\n\n"
        "⚠️ Пожалуйста, при переводе не пишите ничего в комментарии к платежу."
    )
    await callback.message.answer(pay_details, parse_mode="HTML")
    await callback.answer()
...
async def show_my_orders_by_category(callback: CallbackQuery):
    ...
    for o in orders:
        ...
        # --- ДОБАВЛЯЕМ РЕКВИЗИТЫ В ТЕКСТ КАРТОЧКИ ---
        requisites_block = ""
        if curr_status in ["ОЖИДАЕТ ОПЛАТЫ", "ОЖИДАЕТ ОПЛАТЫ ВЕСА"]:
            requisites_block = (
                f"\n📌 <b>Реквизиты для оплаты:</b>\n"
                f"💳 Карта: <code>4444 5555 6666 7777</code>\n"
                f"👤 Получатель: Иван Иванович И.\n"
            )
        ...
        card_text = (
            ...
            f"{requisites_block}" # Вставляем блок реквизитов здесь
            f"━━━━━━━━━━━━━━━━━━\n"
