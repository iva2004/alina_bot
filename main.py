...
# === 4. ЛОКАЛЬНЫЕ МОДУЛИ ПРОЕКТА ===
# База данных и сессии
from database.db_setup import init_db, async_session

# Модели (Все модели в одной скобке, добавили Site и Promotion)
from database.models import (
    Base, Admin, User, Order, Category,
    SiteSetting, GlobalSetting, CartItem, SupportTicket,
    Site, Promotion  # <--- Добавили те, что создали сегодня
)

# Парсер
from parser.engine import get_product_info

# Клавиатуры (Сгруппировали для красоты)
...
class StockStates(StatesGroup):
    waiting_for_cat_name = State()       # Ожидание названия категории
    waiting_for_product_cat = State()    # Выбор категории для товара
    waiting_for_product_photo = State()  # Ожидание фото
    waiting_for_product_desc = State()   # Ожидание описания
    waiting_for_product_size = State()   # Ожидание размера
    waiting_for_product_price = State()  # Ожидание цены
    waiting_for_product_currency = State()  # <-- Новый шаг
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    async with async_session() as session:
        ...
        res = await session.execute(select(DBUser).where(DBUser.tg_id == message.from_user.id))
...
async def handle_promo_broadcast(callback: CallbackQuery, bot: Bot):
    ...
    async with async_session() as session:
        ...
        if action == "all":
            # Рассылка всем
            res = await session.execute(select(DBUser.tg_id))
...
async def mail_personal_list(callback: CallbackQuery):
    ...
    async with async_session() as session:
        ...
        result = await session.execute(select(DBUser).limit(10))
        users = result.scalars().all()
...
async def admin_send_ask_text(message: Message, state: FSMContext, bot: Bot):
    ...
    async with async_session() as session:
        order = await session.get(Order, order_id)
        user = await session.execute(select(DBUser).where(DBUser.id == order.user_id))
...
async def process_order_final(message: Message, state: FSMContext, bot: Bot):
    ...
    async with async_session() as session:
        try:
            # 2. ПОИСК ПОЛЬЗОВАТЕЛЯ (Учитываем ваш алиас User as DBUser)
            # Мы ищем запись, где tg_id совпадает с ID того, кто пишет боту
            user_stmt = await session.execute(select(DBUser).where(DBUser.tg_id == message.from_user.id))
            ...
            # Рассылка всем админам (используем ваш DBUser для поиска админов)
            admins_res = await session.execute(select(DBUser.tg_id).where(DBUser.is_admin == True))
...
async def checkout_handler(callback: CallbackQuery, bot: Bot):
    ...
    async with async_session() as session:
        ...
        # 2. ПРОВЕРКА ПОЛЬЗОВАТЕЛЯ: получаем или создаем запись в DBUser
        user_res = await session.execute(
            select(DBUser).where(DBUser.tg_id == user_tg.id)
        )
        db_user = user_res.scalar_one_or_none()

        # Если пользователя почему-то нет в таблице User, создаем его на лету
        if not db_user:
            db_user = DBUser(tg_id=user_tg.id, full_name=user_tg.full_name, is_admin=False)
            session.add(db_user)
