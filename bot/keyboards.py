...
def get_admin_main_kb(new_count: int = 0, promo_count: int = 0):
    ...
    status_text = f"📑 Заказы ({new_count})" if new_count > 0 else "📑 Заказы"
...
# В файле bot/keyboards.py переименуйте функцию в get_user_main_kb
def get_user_main_kb(is_admin: bool = False) -> ReplyKeyboardMarkup:
    builder = ReplyKeyboardBuilder()

    builder.add(KeyboardButton(text="🏘 Товары в наличии"))
    builder.add(KeyboardButton(text="🌍 Товары из Европы и США"))
...
# И для того, чтобы другие части кода не сломались,
# если они используют старое имя get_final_menu_v2:
get_final_menu_v2 = get_user_main_kb


# Просто перенаправляем вызов
get_user_main_kb = get_final_menu_v2
